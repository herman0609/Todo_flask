...
def register():
    ...
    item = Item(body=_('Witness something truly majestic'), author=user, category_id=0, desc='desc', end_time=end_time)
    item2 = Item(body=_('Help a complete stranger'), author=user, category_id=0, desc='desc', end_time=end_time)
    item3 = Item(body=_('Drive a motorcycle on the Great Wall of China'), author=user, category_id=0, desc='desc', end_time=end_time)
    item4 = Item(body=_('Sit on the Great Egyptian Pyramids'), done=True, author=user, category_id=0, desc='desc', end_time=end_time)
    db.session.add_all([item, item2, item3, item4])
    db.session.commit()

    return jsonify(username=username, password=password, message=_('Generate success.'))

@auth_bp.route('/register0', methods=['GET', 'POST'])
def register0():
    if request.method == 'POST':
        data = request.get_json()
        username = data['username0']
