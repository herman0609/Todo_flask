...
$(document).ready(function () {
    ...
    function new_item(e) {
        ...
        var value = $input.val().trim();
        if (e.which !== ENTER_KEY || !value || !category_id || !desc || !end_time) {
            return;
        }
        $input.focus().val('');
        $('#category_id').val('');
    ...
    $(document).on('click', '#quick-item-btn', quick_new_item.bind(this));


    $(document).on('keyup', '#desc', new_item.bind(this));
    ...
    $(document).on('keyup', '#keyword', function (e) {
        ...
        $items.filter(function () {
            ...
            return ($(this).data('body') + '').indexOf(keyword.val()) == -1;
        }).hide();
    });
    ...
    $(document).on('click', '#active-item', function () {
        ...
        $items.show();
        $items.filter(function () {
            return $(this).data('done') == 1 || $(this).data('done') == 2;
        }).hide();
    });
    ...
    $(document).on('click', '#completed-item', function () {
        ...
        $items.show();
        $items.filter(function () {
            return $(this).data('done') == 0 || $(this).data('done') == 2;
        }).hide();
    });

    $(document).on('click', '#overtime-item', function () {
        var $input = $('#item-input');
        var $items = $('.item');
        ...
        $items.show();
        $items.filter(function () {
            return $(this).data('done') == 0 || $(this).data('done') == 1;
        }).hide();
    });
